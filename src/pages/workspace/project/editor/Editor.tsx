...
function Editor() {
  ...
  const GenerateSlides = useCallback(async () => {
    ...
    console.log(
      "🚀 Starting slide generation based on outline from database..."
    );
    ...
    try {
      ...
      for (let index = 0; index < slidesToGenerate; index++) {
        ...
        if (!outlineItem) {
          console.warn(
            `⚠️ Outline item at index ${index} is undefined, skipping...`
          );
          continue;
        }
        ...
        if (!outlineItem.slidePoint && !outlineItem.outline) {
          console.warn(
            `⚠️ Slide ${index + 1} has no slidePoint or outline, using defaults`
          );
        }
        ...
        console.log(
          `  📝 Content: "${slideOutline.substring(0, 150)}${
        ...
        console.log(
          `  ✅ All outline fields (slideNo, slidePoint, outline) are being used in the prompt`
        );
  ...
  useEffect(() => {
    ...
    if (projectDetail.designStyle) {
      ...
      if (typeof projectDetail.designStyle === "string") {
        console.error(
          "  - ⚠️ WARNING: designStyle is a string, not an object! This is the problem."
        );
      } else {
        console.log(
    ...
    if (
      ...
      console.warn(
        "⚠️ designStyle is a string, not an object. Refetching data..."
      );
      setTimeout(() => {
        GetProjectDetail();
      }, 1000);
    ...
    if (
      ...
      console.log(
        "✅ All conditions met! Starting automatic slide generation..."
      );
      ...
      if (!projectDetail.designStyle) {
        ...
        console.error(
          "  ❌ designStyle is a string (old format), not an object!"
        );
      }
      console.log(
        "💡 You can manually trigger generation using the 'Generate Slides' button"
      );
    }
  ...
  const GeminiSlideCall = async (prompt: string, index: number) => {
    try {
      ...
      for await (const message of session.receive()) {
        messageCount++;
        console.log(
          `📨 Received message ${messageCount} for slide ${index + 1}, type: ${
            message.type
          }`
        );
  ...
  const exportAllIframesToPPT = async () => {
    ...
    try {
      ...
      for (let i = 0; i < iframes.length; i++) {
        ...
        console.log(
          `📸 Converting slide ${i + 1}/${iframes.length} to image...`
        );
    ...
    <div>
      <div className="flex items-center justify-center mt-40">
        <Alert className="max-w-lg">
          ...
          <AlertDescription>
            All slides from your outline will be generated at 800x500
            resolution.
          </AlertDescription>
        </Alert>
      </div>
              ...
              projectDetail?.designStyle && (
                <div className="flex flex-col items-center justify-center p-10 gap-4">
                  ...
                  <Button
                    onClick={() => {
                      ...
                      if (
                        ...
                        alert(
                          "Missing outline or design style. Please check the console."
                        );
                      }
                    }}
                    size="lg"
                  ...
                  <p className="text-xs text-gray-400 text-center max-w-md">
                    Click to generate slides based on your outline and selected
                    design style.
                  </p>
                </div>
              )}
            ...
            {!generatingSlides &&
              ...
              (!projectDetail?.outline ||
                ...
                <div className="flex flex-col items-center justify-center p-10 gap-2">
                  <p className="text-gray-500 text-center">
                    Cannot generate slides.
                  </p>
                  {!projectDetail?.outline ||
                  projectDetail.outline.length === 0 ? (
                    <p className="text-sm text-red-500">
                      ❌ Missing outline. Please go back and generate outline
                      first.
                    </p>
                  ) : null}
                  {!projectDetail?.designStyle ? (
                    <p className="text-sm text-red-500">
                      ❌ Missing design style. Please go back and select a
                      design style.
                    </p>
                  ) : null}
