        ...
        <nav className="fixed top-0 left-0 right-0 z-50 px-4">
            <div className="mx-auto max-w-7xl p-3">
                <div className="flex items-center justify-between mt-2 p-3 md:p-4 rounded-3xl bg-white/30 dark:bg-neutral-900/40 backdrop-blur-md border border-white/10 dark:border-neutral-800/40 shadow-sm">
                            ...
                            <img src={testImage} alt="Logo" width={60} height={30} className="object-contain" />
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                        <Link to="/get-started" className="hidden sm:inline-flex px-5 py-2 rounded-full bg-white text-black font-medium shadow-sm">Login</Link>
                ...
                {open && (
                    <div className="md:hidden mt-2 rounded-xl bg-white/30 dark:bg-neutral-900/40 backdrop-blur-md border border-white/10 dark:border-neutral-800/40 p-3 shadow-lg">
                        <ul className="flex flex-col gap-2">
                            {items.map((it, i) => (
                                <li key={i}>
                                    <a href={it.href} className="block px-3 py-2 rounded-md text-black/80 dark:text-white/80 hover:bg-white/50 dark:hover:bg-neutral-800/50">{it.label}</a>
                                </li>
                            ))}
                            <li className="pt-2">
                                <Link to="/get-started" className="block w-full text-center px-4 py-2 rounded-full bg-black text-white">Login</Link>
                            </li>
                        </ul>
                    </div>
                )}
